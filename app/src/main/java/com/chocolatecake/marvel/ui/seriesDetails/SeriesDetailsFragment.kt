...
import com.chocolatecake.marvel.ui.seriesDetails.adapters.SeriesAdapter
...
class SeriesDetailsFragment : BaseFragment<FragmentSeriesDetailsBinding, SeriesDetailsViewModel>() {
    ...
    private fun setAdapter() {
        ...
        binding.recyclerViewFragmentSeriesDetails.adapter = adapter
    }
    ...
    private fun updateItems() {
        viewModel.events.observe(viewLifecycleOwner) { status ->
            status.toData()?.let {
                adapter.setItem(SeriesDetailsItem.EventsItem(it))
            }
        }
        viewModel.comics.observe(viewLifecycleOwner) { status ->
            status.toData()?.let {
                adapter.setItem(SeriesDetailsItem.ComicsItem(it))
            }
        }
        viewModel.series.observe(viewLifecycleOwner){
            it.toData()?.let {
                adapter.setItem(SeriesDetailsItem.SeriesItem(it))
            }

        }
        viewModel.characters.observe(viewLifecycleOwner) { status ->
            status.toData()?.let {
                adapter.setItem(SeriesDetailsItem.CharactersItem(it))
            }
        }
    }
